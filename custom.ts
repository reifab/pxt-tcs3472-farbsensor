...
const TCS34725_REGISTER_COMMAND_TYPE_AUTOINC = 0x20 //Auto-increment protocol transaction
...
enum TCS34725_AGAIN {
    EMPFINDLICHKEIT_1X = 0x0,      // 1x gain
    EMPFINDLICHKEIT_4X = 0x1,      // 4x gain
    EMPFINDLICHKEIT_16X = 0x2,      // 16x gain
    EMPFINDLICHKEIT_60X = 0x3       // 60x gain
}
...
namespace Farbsensor {
    ...
    //% blockId="start_colorSensor" block="Starte Sensor mit %gain"
    //% gain.defl=TCS34725_AGAIN.EMPFINDLICHKEIT_60X
    export function start(gain: TCS34725_AGAIN) {
        let atime = TCS34725_ATIME.TIME_2_4_MS;
        while (!isConnected) {
            initSensor();
        }
    ...
    export function getSensorRGB(): RGBC {
        ...
        let redColorValue = RegisterHelper.readRegisterUInt16(TCS34725_I2C_ADDR, TCS34725_REGISTER_COMMAND | TCS34725_REGISTER_RDATAL | TCS34725_REGISTER_COMMAND_TYPE_AUTOINC);

        //REGISTER FORMAT:   CMD | TRANSACTION | ADDRESS
        //REGISTER READ:     TCS34725_REGISTER_COMMAND (0x80) | TCS34725_REGISTER_GDATAL (0x18)          
        let greenColorValue = RegisterHelper.readRegisterUInt16(TCS34725_I2C_ADDR, TCS34725_REGISTER_COMMAND | TCS34725_REGISTER_GDATAL | TCS34725_REGISTER_COMMAND_TYPE_AUTOINC);

        //REGISTER FORMAT:   CMD | TRANSACTION | ADDRESS
        //REGISTER READ:     TCS34725_REGISTER_COMMAND (0x80) | TCS34725_REGISTER_BDATAL (0x1A)          
        let blueColorValue = RegisterHelper.readRegisterUInt16(TCS34725_I2C_ADDR, TCS34725_REGISTER_COMMAND | TCS34725_REGISTER_BDATAL | TCS34725_REGISTER_COMMAND_TYPE_AUTOINC);

        //REGISTER FORMAT:   CMD | TRANSACTION | ADDRESS
        //REGISTER READ:     TCS34725_REGISTER_COMMAND (0x80) | TCS34725_REGISTER_CDATAL (0x14)          
        let clearColorValue = RegisterHelper.readRegisterUInt16(TCS34725_I2C_ADDR, TCS34725_REGISTER_COMMAND | TCS34725_REGISTER_CDATAL | TCS34725_REGISTER_COMMAND_TYPE_AUTOINC);

        pauseSensorForIntegrationTime(atimeIntegrationValue);

        //serial.writeValue("r", redColorValue);
        //serial.writeValue("g", greenColorValue);
        //serial.writeValue("b", blueColorValue);
        //serial.writeValue("c", clearColorValue);
        let r = 0;
        let g = 0;
        let b = 0;
        let c = 0;

        r = Math.round(Math.map(redColorValue, 0, 1024, 0, 255));
        g = Math.round(Math.map(greenColorValue, 0, 1024, 0, 255));
        b = Math.round(Math.map(blueColorValue, 0, 1024, 0, 255));
        c = Math.round(Math.map(clearColorValue, 0, 1024, 0, 255));
        return {
            red: r,
            green: g,
            blue: b,
    ...
    //% blockId="getSensorData" block="lese Farbwerte %colorId"
    export function getSensorData(colorId: RGB): number {
        ...
        switch (colorId) {
            ...
            case RGB.BLAU: color = data.blue;
                break;
            case RGB.ALLE_FARBEN: color = data.clear;
                break;
        }
